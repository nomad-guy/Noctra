...
class _MockThrowingResolver implements StreamResolver {
  ...
  @override
  Future<bool> canResolve(Song song) async => true;
...
void main() {
  ...
  group('TrackMatchingGuard — Dangerous Cases & Precision Verification', () {
    ...
    test('Official Audio / Music Video suffix does NOT reject matching song', () {
      final safe = TrackMatchingGuard.isSafeMatch(
        ...
        targetArtist: targetArtist,
        targetDuration: targetDuration,
        candidateTitle: 'The Weeknd - Starboy (Official Music Video) ft. Daft Punk',
  ...
  group('CompositeStreamResolver — Offline Fast-Fail & Host Security', () {
    test('Offline SocketException immediately terminates resolver traversal without waiting for subsequent tiers', () async {
      final throwing = _MockThrowingResolver(const SocketException('Failed host lookup'));
      ...
      expect(neverReached.callCount, 0, reason: 'Must not attempt further tiers when offline');
    });
    ...
    test('DirectOpenStreamResolver accepts Jamendo hosts', () async {
      ...
      final jamendoCdnSong = Song(
        ...
        artist: 'Creative Commons',
        duration: const Duration(seconds: 180),
        streamUrl: 'https://prod-1.storage.jamendocdn.com/stream/track/12345.mp3',
      );
  ...
  group('PlayerQueueMixin.onSongDownloaded — Hot Swapping & Queue In-Place Update', () {
    test('Updates currentSong and queue with localFilePath and isDownloaded = true', () {
      ...
      svc.debugSetPlaybackPosition(queue: [songOnline, otherSong], index: 0, currentSong: songOnline);
      ...
      expect(svc.currentSong?.localFilePath, '/storage/emulated/0/Music/Noctra/song-dl-1.m4a');
      expect(svc.queue[0].isDownloaded, isTrue);
      expect(svc.queue[0].localFilePath, '/storage/emulated/0/Music/Noctra/song-dl-1.m4a');
