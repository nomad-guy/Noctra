...
class _MigrationScreenState extends ConsumerState<MigrationScreen> {
  ...
  Widget _buildContent(bool isDark) {
    switch (_step) {
      case MigrationStep.choose:
        return _buildChooseStep(isDark);
      case MigrationStep.loading:
        return _buildLoadingStep(isDark);
      ...
      children: [
        ...
        ..._sources.map((s) => Padding(
              ...
              child: GestureDetector(
                onTap: () => _selectSource(s['source'] as String),
                child: GlassCard(
                  radius: 14,
                  padding:
      ...
      child: Column(
        ...
        children: [
          CircularProgressIndicator(
              strokeWidth: 2, color: isDark ? Colors.white : Colors.black),
          const SizedBox(height: 16),
          Text(_status,
              style: TextStyle(
                  fontSize: 13,
    ...
    return ListView(
      ...
      children: [
        ...
        if (_matchedTracks.where((m) => m.isUncertain).isNotEmpty) ...[
          Text('Possible Matches (Review)',
              style: TextStyle(
                  ...
                  color: isDark ? Colors.white70 : Colors.black87)),
          const SizedBox(height: 8),
          ..._matchedTracks.where((m) => m.isUncertain).take(10).map(
                (m) => GlassCard(
                  radius: 10,
        ...
        Row(
          children: [
            Expanded(
                child: _actionButton('Cancel', isDark,
                    onTap: () => setState(() => _step = MigrationStep.choose))),
            const SizedBox(width: 12),
            Expanded(
                child: _actionButton('Import Library', isDark,
                    primary: true, onTap: _commitImport)),
          ],
        ),
        const SizedBox(height: 20),
      ],
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
              '${_matchedTracks.where((m) => m.isMatched).length} tracks added to your library',
              style: TextStyle(
                  fontSize: 13,
                  color: isDark ? Colors.white54 : Colors.black54)),
          const SizedBox(height: 24),
          _actionButton('Done', isDark,
  ...
  void _selectSource(String sourceName) async {
    ...
    // Keep the import interface provider-neutral. The importer is selected by
    // its opaque source key, but no provider branding is exposed here.
    if (mounted) {
      showDialog(
        context: context,
