...
class ArtistMetadataService {
  ...
  static Future<ArtistMetadata> fetchArtistInfo(String artistName) async {
    ...
    try {
      ...
      final res = await http.get(uri, headers: {'User-Agent': 'Mozilla/5.0'}).timeout(const Duration(milliseconds: 2500));
      if (res.statusCode == 200) {
        ...
        if (list != null && list.isNotEmpty) {
          ...
          if (pic != null && pic.toString().isNotEmpty && !pic.toString().contains('artist-default')) {
    ...
    if (resolvedImageUrl == null || resolvedImageUrl.isEmpty) {
      try {
        final saavnUri = Uri.parse('https://www.jiosaavn.com/api.php?__call=autocomplete.get&_format=json&_marker=0&cc=in&includeMetaTags=1&query=${Uri.encodeComponent(cleanName)}');
        final res = await http.get(saavnUri, headers: {'User-Agent': 'Mozilla/5.0'}).timeout(const Duration(milliseconds: 2500));
        if (res.statusCode == 200) {
          final data = jsonDecode(res.body) as Map<String, dynamic>;
          ...
          if (artists != null && artists.isNotEmpty) {
            for (final item in artists) {
              ...
              if (rawImg.isNotEmpty && !rawImg.contains('artist-default')) {
                resolvedImageUrl = rawImg.replaceAll('50x50', '500x500').replaceAll('150x150', '500x500');
                break;
              }
            }
          }
    ...
    if (resolvedImageUrl == null || resolvedImageUrl.isEmpty) {
      try {
        final itunesUri = Uri.parse('https://itunes.apple.com/search?term=${Uri.encodeComponent(cleanName)}&entity=song&limit=1');
        final res = await http.get(itunesUri, headers: {'User-Agent': 'Mozilla/5.0'}).timeout(const Duration(milliseconds: 2500));
        if (res.statusCode == 200) {
          final data = jsonDecode(res.body) as Map<String, dynamic>;
    ...
    try {
      ...
      final res = await http.get(wikiUri, headers: {'User-Agent': 'NoctraMusicApp/1.0 (contact@noctra.app)'}).timeout(const Duration(milliseconds: 2500));
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body) as Map<String, dynamic>;
        final String? desc = data['description']?.toString().toLowerCase();
        final bool isMusicEntity = desc != null && (desc.contains('singer') || desc.contains('musician') || desc.contains('band') || desc.contains('rapper') || desc.contains('composer') || desc.contains('artist') || desc.contains('producer') || desc.contains('record'));
    ...
    if (bio == null) {
      try {
        final queryUrl = Uri.parse('https://en.wikipedia.org/w/api.php?action=query&generator=search&gsrsearch=${Uri.encodeComponent("$cleanName musician")}&gsrlimit=1&prop=extracts&exintro=1&explaintext=1&format=json&origin=*');
        final res = await http.get(queryUrl, headers: {'User-Agent': 'NoctraMusicApp/1.0 (contact@noctra.app)'}).timeout(const Duration(milliseconds: 2500));
        if (res.statusCode == 200) {
          final data = jsonDecode(res.body) as Map<String, dynamic>;
